...
# Si usas Euclid, ajusta este import según tu instalación de lenstronomy
# try:
#     from lenstronomy.SimulationAPI.ObservationConfig.euclid import Euclid
# except ImportError:
#     Euclid = None  # Se comprobará más abajo si intentas usar EUCLID

# ===========================================================
# USER CONFIG: instrumento, banda y BIC
# ===========================================================
INSTRUMENT   = "EUCLID"   # "LSST" o "EUCLID"
LSST_BAND    = "g"      # u,g,r,i,z,y (según LSST)
EUCLID_BAND  = "VIS"    # típicamente "VIS" o "NIR"
COMPUTE_BIC  = True   # True: calcula chi2/BIC; False: no
...
# ===========================================================
# INSTRUMENT CONFIG (LSST o EUCLID)
# ===========================================================
def make_single_band_config():
    inst_name = INSTRUMENT.upper()
    if inst_name == "LSST":
        inst = LSST(band=LSST_BAND, psf_type='GAUSSIAN', coadd_years=10)
        band_label = f"LSST-{LSST_BAND}"
    elif inst_name == "EUCLID":
        if Euclid is None:
            raise RuntimeError("No se pudo importar Euclid. Ajusta el import según tu versión de lenstronomy.")
        inst = Euclid(band=EUCLID_BAND, coadd_years=6)
        band_label = f"EUCLID-{EUCLID_BAND}"
    else:
        raise ValueError(f"INSTRUMENT debe ser 'LSST' o 'EUCLID', no '{INSTRUMENT}'.")

    kwargs_band = inst.kwargs_single_band()
    return kwargs_band, band_label
...
# BIC SETTINGS
# ===========================================================
K_SUB   = 7   # EPL + SIS + SHEAR_REDUCED
K_NOSUB = 4   # EPL + SHEAR_REDUCED
...
# ===========================================================
def simulate_single_band(m_sub, pos_sub, pos_src, compute_bic=COMPUTE_BIC):
...
if __name__ == "__main__":
    ...
    bic_tag = "BIC" if COMPUTE_BIC else "noBIC"
    OUT_NAME = f"{INSTRUMENT.upper()}_{BAND_LABEL}_singleband_{bic_tag}.h5"

    with h5py.File(OUT_NAME, "w") as f:
        # Core images
        d_img_obs   = f.create_dataset("image_sub",            (N_TOTAL, NUMPIX, NUMPIX), dtype='f4', compression='gzip', compression_opts=4)
        d_sub_clean = f.create_dataset("image_sub_nonoise",    (N_TOTAL, NUMPIX, NUMPIX), dtype='f4', compression='gzip', compression_opts=4)
        d_nosub_clean = f.create_dataset("image_nosub_nonoise",(N_TOTAL, NUMPIX, NUMPIX), dtype='f4', compression='gzip', compression_opts=4)
        ...
        # Scalars: chi2, chi2_red, BICs (solo si COMPUTE_BIC=True)
        if COMPUTE_BIC:
        ...
        for i in range(N_TOTAL):
            ...
            # Metrics (solo si COMPUTE_BIC=True)
            if COMPUTE_BIC:
            ...
            if (i+1) % 50 == 0:
                if COMPUTE_BIC:
                    ...
                    print(f"[{i+1}/{N_TOTAL}] sample done")

        # file attrs
        f.attrs['description'] = (
            f"Single-band simulated lensing dataset ({BAND_LABEL}, SUB world obs) "
            f"with{'out' if not COMPUTE_BIC else ''} BIC/chi2 for sub vs nosub models."
        )
        ...
        if COMPUTE_BIC:
            ...
            f.attrs['delta_bic']      = "dBIC = BIC_nosub - BIC_sub (positive values favor SUB model)"
