...
def simulate_pair_and_delta(mass_subhalo, position_subhalo, source_position):
    ...
    kwargs_source_mag_g = [{'magnitude': mag_g, 'R_sersic': R_sersic, 'n_sersic': n_sersic, 'center_x': x_src, 'center_y': y_src}]
...
# --- Rango de masas del subhalo ---
mass_min, mass_max = 1e7, 1e9  # [M_sun]

# --- Rango de posiciones del subhalo (x, y) en arcsec ---
subhalo_pos_min, subhalo_pos_max = -1.6, 1.6

# --- Rango de posiciones de la fuente (x, y) en arcsec ---
source_pos_min, source_pos_max = -0.3, 0.3  # ejemplo más contenido
...
_example_rgb, _example_clean, _example_delta = simulate_pair_and_delta(
    ...
    source_position=(0.0, 0.0)  # ejemplo fijo
)
ny, nx, n_channels = _example_rgb.shape  # asume RGB = 3 canales

# --- Crear archivo HDF5 ---
with h5py.File('LSST_Mmxy.h5', 'w') as f:
    ...
    dset_src_x       = f.create_dataset('source_x',        (N,),                    dtype='f4')
    dset_src_y       = f.create_dataset('source_y',        (N,),                    dtype='f4')
    ...
    for i in range(N):
        ...
        # Muestreo de posición del subhalo
        x_sub = np.random.uniform(subhalo_pos_min, subhalo_pos_max)
        y_sub = np.random.uniform(subhalo_pos_min, subhalo_pos_max)

        # Muestreo de posición de la fuente
        x_src = np.random.uniform(source_pos_min, source_pos_max)
        y_src = np.random.uniform(source_pos_min, source_pos_max)
    ...
    f.attrs['subhalo_pos_range'] = (subhalo_pos_min, subhalo_pos_max)
    f.attrs['source_pos_range']  = (source_pos_min, source_pos_max)
    f.attrs['description'] = ("Simulated RGB gravitational lensing dataset (LSST-like): "
                              ...
                              "Labels include subhalo mass and (x,y) positions for subhalo and source.")
